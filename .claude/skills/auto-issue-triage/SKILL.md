...
Automatically identifies, prioritizes, and begins working on the most severe GitHub issues in this standalone Rust q linter.
...
- Run `cargo test && cargo clippy --all-targets -- -D warnings && cargo fmt --check` to confirm or refute reproducibility before concluding it is a genuine library bug

**Verification Points:**
- Bug claims must match actual code logic
- Wrong line numbers indicate misunderstandings
- Feature requests require checking existing rules (`qlinter --rules`) first
...
3. Implement fix inside that worktree and run `cargo test && cargo clippy --all-targets -- -D warnings && cargo fmt --check` 
...
- Test all fixes with the existing Rust test suite (`cargo test`) before closing issues
