...
| "Refactor X" | "Run `cargo test && cargo clippy --all-targets -- -D warnings && cargo fmt --check` before and after; ensure identical pass count" |
...
In this codebase, "verify" almost always means: run the full test suite (`cargo test && cargo clippy --all-targets -- -D warnings && cargo fmt --check`, and clippy if available via `export QHOME=~/.kx PATH="$HOME/.kx/bin:$PATH"`) before considering a change done.
