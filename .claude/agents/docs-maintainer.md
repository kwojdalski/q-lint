...
- **Never edit build output.** `target/`, `dist/` and
  `editors/vscode/out/` are produced by a build; change the source.
- **The rule catalogue is the code.** `--rules` and `--explain` print from
  `src/taxonomy.json` and the `RULES` table, so a rule's description is
  changed there and not in prose that repeats it.
